...
void setNextEvent(const HatchEvent &event) {
  Serial.println("Hatch close " + timet2str(event.openTime));
  Serial.println("Hatch open " + timet2str(event.closeTime));
...
void addEvents() { 
  // #2 reversed analemmas

  // Analemma
  addEvent(9, 58, 3 * SECS_PER_MIN);
  addEvent(10, 57, 6 * SECS_PER_MIN);
  addEvent(11, 55, 10 * SECS_PER_MIN);
  addEvent(12, 57, 6 * SECS_PER_MIN);
  addEvent(13, 58, 3 * SECS_PER_MIN);
}
...
void makeSchedule() {  
  ...
  Serial.println("Next close  " + timet2str(nextEvent.openTime));
  Serial.println("Next open " + timet2str(nextEvent.closeTime));  
}
...
void readJournal() {
  ...
  while (i + sizeof(record) < EEPROM.length()) {  
    ...
    Serial.println(" close: " + timet2str(record.openTime) + " open: " + timet2str(record.closeTime));
  }
